...
int main(int argc, char *argv[]) {
	// Check to make sure a parameter was given
	if (argv[1] == NULL) {
	...
	while ((dir_entry = readdir(dir)) != NULL) {
		...
		if (dir_entry->d_type == 8) {
			...
			// Concatinate the directory path and the file name
			char *path = (char *) malloc(1+strlen(argv[1])+strlen(dir_entry->d_name));
			if (path == 0) {
			...
			if (file == NULL) {
				...
				// Convert the CRC uint32_t to a string
				// and store it into the files structure
				sprintf(files[i][1],"%08X",crc32(crc, contents, size));
