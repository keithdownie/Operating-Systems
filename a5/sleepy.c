// Modified by Keith Downie for Assignment 5
	    ...
	    loff_t *f_pos)
{
  ...
  // Grab the lock for changing the flag
  // Want to make sure flag isn't set back to 0 while processes wake
  if (mutex_lock_killable(&dev->flag_mutex)) {
    // Don't want to accidentally keep holding the lock
    mutex_unlock(&dev->sleepy_mutex);
    return -EINTR;
  }
  // Set the flag to 1 so processes wake up
  dev->flag = 1;
  // Interrupt waiting processes
  wake_up_interruptible(&dev->queue);
  // Unlock the flag mutex
  mutex_unlock(&dev->flag_mutex);
         ...
         loff_t *f_pos)
{
  ...
  // Make sure the number of bytes is 4
  if (count != 4) {
    return -EINVAL;
  }

  // Copy the data that the user has written
  if (copy_from_user(&dptr, buf, count)) {
    // Error occured
    // Don't want to accidentally keep holding the lock
    mutex_unlock(&dev->sleepy_mutex);
    return -EFAULT;
  ...
  // If the integer is negative, do not wait
  // Just return 0
  if (dptr < 0) {
    return retval;
  }

  // Calculate how long it should wait
  duration = dptr*HZ;
  // Calculate when it should wake up
  end_time = jiffies + duration;
  // Grab the flag mutex
  if (mutex_lock_killable(&dev->flag_mutex)) {
    mutex_unlock(&dev->sleepy_mutex);
    return -EINTR;
  }
  // Set the flag to 0 so the process doesn't wake
  dev->flag = 0;
  // Release the flag mutex
  mutex_unlock(&dev->flag_mutex);
  // Release the sleepy mutex before going into timeout!
  mutex_unlock(&dev->sleepy_mutex);

  // Go to sleep until dev->flag == 1, directly woken, or the duration has passed
  if (wait_event_interruptible_timeout(dev->queue, dev->flag != 0, duration)) {
    // Woken up prematurely, return the amount of time left
    retval = (end_time-jiffies)/HZ;
  }
